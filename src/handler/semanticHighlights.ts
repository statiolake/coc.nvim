...
export default class SemanticHighlights {
  ...
  public async semanticHighlight(doc: Document): Promise<boolean> {
    ...
    const curr = await this.getHighlights(doc, this.tokenSource)
  ...
  public async getHighlights(
    ...
    try {
      const relatives = await this.getRelativeHighlights(doc, cancelToken)
      let res: Highlight[] = []
      let currentLine = 0
  ...
  private async getRelativeHighlights(
    ...
    try {
      ...
      const { token: cancelToken } = tokenSource
      doc.forceSync()
      const legend = await languages.getLegend(doc.textDocument)
      const tokens = await languages.provideDocumentSemanticTokens(
        doc.textDocument,
        cancelToken
      )
